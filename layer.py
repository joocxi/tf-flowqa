            ...
            for layer in range(self.num_layers):
                if self.bidirectional:
                    ...
                    with tf.variable_scope("fw_{}".format(layer)):
                        out_fw, _ = tf.nn.dynamic_rnn(rnn_fw, outputs[-1] * mask_fw, seq_len, dtype=tf.float32)
                    with tf.variable_scope("bw_{}".format(layer)):
                        inputs_bw = tf.reverse_sequence(
                            outputs[-1] * mask_bw, seq_lengths=seq_len, seq_dim=1, batch_dim=0)
                        out_bw, _ = tf.nn.dynamic_rnn(
                            rnn_bw, inputs_bw, seq_len, dtype=tf.float32)
                        out_bw = tf.reverse_sequence(
                            out_bw, seq_lengths=seq_len, seq_dim=1, batch_dim=0)
                    outputs.append(tf.concat([out_fw, out_bw], axis=2))
                else:
                    rnn_fw = self.rnns[layer][0]
                    #init_fw = self.inits[layer]
                    mask_fw = self.dropout_mask[layer][0]
                    with tf.variable_scope("fw_{}".format(layer)):
                        out_fw, _ = tf.nn.dynamic_rnn(
                            rnn_fw, outputs[-1] * mask_fw, seq_len, dtype=tf.float32)
...
def question_attention(context, questions, cache):
    """
    Parameters
    ...
    """
    batch_size, turn_size, para_size, ques_size, glove_dim = cache
...
def fully_aware_attention(context_how, questions_how, questions, cache, scope):
    """
    :context_how, (batch_size, turn_size, para_size, context_how_dim)
    :questions_how, (batch_size, turn_size, ques_size, question_how_dim)
    :questions, (batch_size, turn_size, ques_size, ques_dim)

    :res, (batch_size, turn_size, para_size, ques_dim)
    """
    batch_size, turn_size, ques_size, attention_dim = cache
    ...
    with tf.variable_scope(scope):
        ...
        questions_fc = tf.reshape(flat_questions_fc, [batch_size, turn_size, ques_size, attention_dim])
        # shape: (batch_size, turn_size, para_size, ques_size)
        outputs = tf.matmul(context_fc, tf.transpose(questions_fc, [0, 1, 3, 2]))
        weights = tf.nn.softmax(outputs)

        # shape: (batch_size, turn_size, para_size, ques_dim)
        res = tf.matmul(weights, questions)
        return res


def integration_flow(context, context_len, turn_len,  cache, is_train, scope):
    """
    :context, (batch_size, turn_size, para_size, context_dim)

    Returns
    :c_next, (batch_size, turn_size, para_size, 3*hidden_dim)
    """
    batch_size, turn_size, para_size, context_dim, hidden_dim = cache

    _context = tf.reshape(context, [batch_size*turn_size, para_size, context_dim])


    with tf.variable_scope(scope):
        with tf.variable_scope("integration"):
            integration = rnn(num_layers=1, bidirectional=True, num_units=hidden_dim,
        batch_size=batch_size*turn_size, input_size=context_dim, is_train=is_train)

            # shape: (batch_size*turn_size, para_size, 2*hidden_dim)
            c_hat = integration(_context, seq_len=tf.tile(context_len, [turn_size]))
        ...
        with tf.variable_scope("flow"):
            ...
            f = flow(_c_hat, seq_len=tf.tile(turn_len, [para_size]))
